...
def run_grounded_sam(input_image, text_prompt, task_type, inpaint_prompt, box_threshold, text_threshold, iou_threshold, inpaint_mode, scribble_mode, openai_api_key):
    ...
    image = input_image["composite"]
    # scribble = input_image["mask"]
    scribble = input_image["layers"]
    size = image.size # w, h
...
if __name__ == "__main__":
    ...
    with block:
        with gr.Row():
            with gr.Column():
                input_image = gr.ImageEditor(sources='upload', type="pil", value="assets/demo2.jpg")#, tool="sketch")
    ...
    block.launch(server_name='0.0.0.0', server_port=args.port, debug=args.debug, share=True)#args.share)